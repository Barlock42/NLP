            ...
            for sentence in sents: #doc.sents
                ...
                if mes_language == doc_language:
                    ...
                    if q_entity == df_q_entity:
                        ...
                        if mes_language == 'en':
                            mv_t = np.dot(mv, en_de_trans_model.translation_matrix)
                            doc_similarity = cos_sim(mv_t, sv)
                            if doc_similarity < (1 - sim_coef)/2:
                                ...
                                for word in mes_no_stop_words:
                                    if nlp_en.vocab.get_vector(word.text)[0] != 0: #TODO Learn new word vectors from context
                                        if word.text not in en_list:
                                            temp = translator.translate(word.text, src='en', dest='de').text
                                            # result = translate.translate_text(Text=word.text, SourceLanguageCode="en", TargetLanguageCode="de")
                                            # translation = result.get('TranslatedText')
                                            if nlp_de.vocab.get_vector(temp)[0] != 0:
                                ...
                                en_de_trans_model.train(word_list)
                                mv_t = np.dot(mv, en_de_trans_model.translation_matrix)
                                doc_similarity = cos_sim(mv_t, sv)
                                if doc_similarity >= sim_coef:
                                    index.append([doc_similarity, sentence, j_o['question'], j_o['answer']])
                            elif doc_similarity >= sim_coef:
                                index.append([doc_similarity, sentence, j_o['question'], j_o['answer']])
                        else:
                            ...
                            if doc_similarity < (1 - sim_coef)/2:
                                ...
                                for word in mes_no_stop_words:
                                    if nlp_de.vocab.get_vector(word.text)[0] != 0: #TODO Learn new word vectors from context
            ...
            if doc_conc_count == len(doc_speech_parts.nouns):
                ...
                if q_entity == "yes/no":
                    ...
                    return par + "According to: " + "question: " + candidate[2] + "; answer: " + candidate[
                        3] + '\n' + str(mes_no_stop_words.similarity(sent_no_stop_words)), True
        else:
            doc_conc_count = concurrences_count(mes_id_objects, doc_id_objects, mes_language,
                                                doc_language, True)

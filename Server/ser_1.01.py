...
def version_resolution(mes, mes_speech_parts):
    ...
    for noun in mes_speech_parts.nouns:
        ...
        for i in range(mes_pos, len(mes) - 1):#TODO Work with noun chunks
    ...
    for j_o in df:
        if hasattr(objects, name):
            ...
            for sentence in sents: #doc.sents
                sent = nlp_d(sentence)
                #df_q_entity = question_resolution(sent, doc_language, True) #TODO Filtering of candidates
    ...
    for candidate in index:
        ...
        if conc_count >= len(mes_speech_parts.nouns):
            ...
            if doc_conc_count == len(doc_speech_parts.nouns):
                ...
                for obj in vars(doc_id_objects):
                    if hasattr(doc_id_objects[obj], "version"):
                        if isinstance(doc_id_objects[obj]["version"], int) or isinstance(doc_id_objects[obj]["version"], float):
            ...
            if doc_conc_count >= len(mes_speech_parts.nouns):
                ...
                for obj in vars(doc_id_objects):
                    if hasattr(doc_id_objects[obj], "version"):
                        if isinstance(doc_id_objects[obj]["version"], int) or isinstance(doc_id_objects[obj]["version"],
                                                                                         float):
                            par += doc_id_objects[obj]["name"] + doc_id_objects[obj][
                                "version"] + ' '
                        else:
    ...
    if q_entity:
        ...
        if not isinstance(answer, str):
            ...
            if not hasattr(answer, q_entity):
                ...
                most_possible_coef = 0.85

                if most_possible:
                    most_possible = sorted(most_possible, reverse=True)
                    if most_possible[0][0] >= most_possible_coef:
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        req_start = time.time()
        r = requests.post("https://analytics.liebl.de/aqc?asin=" + asin + "&language=" + mes_language)
...
with socketserver.ThreadingTCPServer(("", PORT), SimpleHTTPRequestHandler) as httpd:#Threading
    print("serving at port", PORT)

    print("Loading spacy models... (≈60sec.)")
    start = time.time()
    ...
    print("Loading language spaces... (≈200sec.)")
    start = time.time()
    en_space = Space.build(nlp_en)
    end_en = time.time()
    print("English language space loaded...")
    print(end_en - start, " seconds")
    start_de = time.time()
    de_space = Space.build(nlp_de)
    end = time.time()
    print("German language space loaded...")
    print(end - start_de, " seconds")
